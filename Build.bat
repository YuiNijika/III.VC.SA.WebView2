...
echo ==========================================
echo    III.VC.SA.WebView2 Builder
echo ==========================================
...
if not exist "build\WebView2.sln" (
    echo [Error] build\WebView2.sln not found after generation.
    goto fail
)
...
for %%P in (WebView2 WebView2PayloadSA WebView2PayloadVC WebView2PayloadIII) do (
...
rem Loader and payloads must not link platform or renderer libraries directly.
findstr /I /M /C:"d3d9" /C:"Direct3D" /C:"urlmon.lib" /C:"Pdh.lib" /C:"shell32.lib" /C:"ole32.lib" "build\WebView2.vcxproj" "build\WebView2Payload*.vcxproj" >nul 2>nul
if not errorlevel 1 (
    echo [Error] Generated runtime projects still link platform or renderer libraries directly.
    exit /b 1
)

findstr /I /C:"XBaseBootstrap.lib" "build\WebView2.vcxproj" >nul 2>nul
if errorlevel 1 (
    echo [Error] Loader project does not link XBaseBootstrap.lib.
    exit /b 1
)
findstr /I /C:"/WHOLEARCHIVE:XBaseBootstrap.lib" "build\WebView2.vcxproj" >nul 2>nul
if errorlevel 1 (
    echo [Error] Loader project does not retain the XBase bootstrap entry object.
    exit /b 1
)
for %%G in (SA VC III) do (
    findstr /I /C:"XBase%%G.lib" "build\WebView2Payload%%G.vcxproj" >nul 2>nul
    if errorlevel 1 (
        echo [Error] Payload project %%G does not link XBase%%G.lib.
        exit /b 1
    )
    findstr /I /C:"XBasePayloadEntry.lib" "build\WebView2Payload%%G.vcxproj" >nul 2>nul
    if errorlevel 1 (
        echo [Error] Payload project %%G does not link XBasePayloadEntry.lib.
        exit /b 1
    )
    findstr /I /C:"/WHOLEARCHIVE:XBasePayloadEntry.lib" "build\WebView2Payload%%G.vcxproj" >nul 2>nul
    if errorlevel 1 (
